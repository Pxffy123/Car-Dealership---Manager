from pathlib import Path
from Utils.storage import load_json, save_json

class Vehicle:
    def __init__(self, vehicle_id, make, model, year, price, mileage, color, status="Available"):
        self.vehicle_id = vehicle_id
        self.make = make
        self.model = model
        self.year = year
        self.price = float(price)  
        self.mileage = mileage
        self.color = color
        self.status = status

    def sell(self):
        if self.status == "Sold":
            return False
        self.status = "Sold"
        return True

    def to_dict(self):
        return {
            "vehicle_id": self.vehicle_id,
            "make": self.make,
            "model": self.model,
            "year": self.year,
            "price": self.price,
            "mileage": self.mileage,
            "color": self.color,
            "status": self.status
        }

    
    @staticmethod
    def from_dict(data):
        return Vehicle(
            vehicle_id=data["vehicle_id"],
            make=data["make"],
            model=data["model"],
            year=data["year"],
            price=data["price"],
            mileage=data["mileage"],
            color=data["color"],
            status=data.get("status", "Available")
        )

    def __str__(self):
        return (
            f"{self.vehicle_id}: "
            f"{self.year} {self.make} {self.model} "
            f"- KSh {self.price:,.2f}"
        )


class CarCollection:
    def __init__(self, cars_file="Data/Vehicles.json"):
        self.cars_file = Path(cars_file)

    def _load_cars(self):
        
        raw_data = load_json(self.cars_file)
        if not raw_data:
            return []
        return [Vehicle.from_dict(item) for item in raw_data]

    def _save_cars(self, cars_list):
        
        save_json(self.cars_file, [car.to_dict() for car in cars_list])

    def add_car(self, car):
        cars = self._load_cars()
        cars.append(car)
        self._save_cars(cars)  
    def view_cars(self):  
        return self._load_cars()

    def search_car(self, search_text):
        
        search_text = search_text.lower()
        return [
            car
            for car in self._load_cars()
            if search_text in car.make.lower()
            or search_text in car.model.lower()
            or search_text in car.vehicle_id.lower()
        ]

    def update_status(self, vehicle_id, new_status):
        
        cars = self._load_cars()
        for car in cars:
            if car.vehicle_id.lower() == vehicle_id.lower():
                car.status = new_status
                self._save_cars(cars)
                return True
        return False

    def delete_car(self, vehicle_id):
        
        cars = self._load_cars()
        for car in cars:
            if car.vehicle_id.lower() == vehicle_id.lower():
                cars.remove(car)
                self._save_cars(cars)
                return True
        return False